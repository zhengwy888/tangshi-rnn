...
local LineSplitLoader = {}
LineSplitLoader.__index = LineSplitLoader

function LineSplitLoader.create(data_dir, batch_size, seq_length, split_fractions)
    -- split_fractions is e.g. {0.9, 0.05, 0.05}

    local self = {}
    setmetatable(self, LineSplitLoader)
    ...
    if run_prepro then
        ...
        LineSplitLoader.text_to_tensor(input_file, vocab_file, tensor_file)
    end
...
function LineSplitLoader:reset_batch_pointer(split_index, batch_index)
    batch_index = batch_index or 0
    self.batch_ix[split_index] = batch_index
end

function LineSplitLoader:next_batch(split_index)
...
-- *** STATIC method ***
function LineSplitLoader.text_to_tensor(in_textfile, out_vocabfile, out_tensorfile)
    ...
    print('putting data into tensor with dimension' .. linecount .. maxCharCount)
    ...
    local data = torch.ShortTensor(linecount, maxCharCount+1)
    local pos = 1
    local linecount = 0
    local currentCharCount = 0
    -- code snippets taken from http://lua-users.org/wiki/LuaUnicode
    for char in string.gfind(rawdata, "([%z\1-\127\194-\244][\128-\191]*)") do
        if char == "\n" then
            -- more hack
            if ( currentCharCount ~= maxCharCount+1 ) then 
                data[linecount+1][currentCharCount+1] = vocab_mapping['s']
            end
            linecount = linecount + 1
            currentCharCount = 0
        else
            -- print(currentCharCount+1 .. ' ' .. pos .. ' ' .. len)
            data[linecount+1][currentCharCount+1] = vocab_mapping[char]
            currentCharCount = currentCharCount + 1
        end
        pos = pos + 1
    end
